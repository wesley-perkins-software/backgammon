...
import { HOME_BOARD, MAX_UNDO, PLAYER_A, PLAYER_B, SCHEMA_VERSION, STARTING_POINTS } from './constants.js';
...
export function isPointOwnedBy(points, index, player) {
  const value = points[index];
  return player === PLAYER_A ? value > 0 : value < 0;
}

export function isPlayerChecker(value, player) {
  return player === PLAYER_A ? value > 0 : value < 0;
}

export function countAt(points, index, player) {
  const value = points[index];
...
export function moveDirection(player) {
  return player === PLAYER_A ? -1 : 1;
}

export function entryIndexFromBar(player, die) {
  return player === PLAYER_A ? 24 - die : die - 1;
}

export function removeDie(remaining, die) {
...
export function uniqueDice(remaining) {
...
export function canUseOversizedBearOff(state, from, die, player) {
  if (player === PLAYER_A) {
    if (die <= from + 1) {
      return false;
    }
...
export function generateBarMoves(state, die, player) {
  if (state.bar[player] <= 0) {
    return [];
  }
...
export function generatePointMoves(state, die, player, fromIndex) {
...
export function generateSingleDieMoves(state, die) {
...
export function exploreMovePaths(state, remainingDice, path = []) {
...
export function maxPlayableMoves(state) {
